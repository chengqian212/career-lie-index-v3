...
14. 如果上一轮已经问过“平时主要忙什。一天做什。日常工作内容”，本轮必须换到新的信息角度，例如方法、判断标准、难点、边界、复盘、产出或成长路径。
15. 如果 priority_issue 表示“职业身份粒度过。职业大类过宽/缺少具体系统、岗位性质或职责方向”，本轮只做职业类型澄清：自然问对方更偏哪个系统、岗位方向、服务对象或职责类型；不要问“怎么处理公务/工作/客户/病人/项目”，也不要直接进入工作流程追问。
...
18.每次提问必须锚定用户上一轮回答中的具体细节或场景。不要从零开始问抽象流程（如"一般是怎么判断先处理哪一步、后处理哪一步"），而要结合对方刚说的内容来问（如"你刚才说到了现场先看刹车痕迹，那要是痕迹特别淡、几乎看不清，你一般会怎么判断"）。
【推荐表达方式。
更推荐这类开放式问法。
